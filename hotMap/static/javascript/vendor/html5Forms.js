...
(function(d){
    ...
    setup = function(form,settings) {
        ...
        var opts = {
			validClass : "valid1",
			invalidClass : "invalid1",
			requiredClass : "required1",
    ...
    checkValidity = function (el) {
        var f, ff, isRequired, hasPattern, hasData, invalid = false;
        ...
        if(el.nodeName.toLowerCase() === "form") {
            ...
            for(var i = 0,len = f.length;i < len;i++) {
                ...
                hasData = !!(!(ff.value === "" || ff.value === placeholder));
				
                if(ff.nodeName.toLowerCase() !== "fieldset" && (isRequired || (hasPattern && hasData))) {
                    checkField(ff);
